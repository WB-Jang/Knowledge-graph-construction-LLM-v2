...
class LegalEntity(BaseModel):
    ...
    full_text: str = Field(description="원문")
    cross_law_refs: List[str] = Field(default=[], description="타 법령(「법명」) 조항 참조 — HGT 다법령 통합 추론 학습용 핵심 특성")
    intra_law_refs: List[str] = Field(default=[], description="본 법령 내부 조항 상호참조 (REFERS_TO 엣지용)")
