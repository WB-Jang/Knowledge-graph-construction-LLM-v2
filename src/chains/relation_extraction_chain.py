...
아래 목록에 있는 값만 "relation" 필드에 사용하세요. 목록에 없는 값은 사용 불가.

- STRUCTURE (구조): 상위조항, 참조함, 준용함, 위임함, 근거함
- LOGICAL (논리): 정의함, 구성함, 예외로함, 제외함, 간주함, 우선함, 조건임, 정함
- ACTION (행위): 요구함, 금지함, 허용함, 보장함, 권한을가짐, 절차를따름, 점검함, 시정함, 심의·의결함, 준수함, 조사함, 제한함, 충족함, 추천함, 만족함
- REPORT (보고/협력): 보고함, 협조함, 통보함
- SANCTION (제재): 처벌대상임, 과태료대상임, 책임이있음, 위반함
- SCOPE (적용): 적용대상임
